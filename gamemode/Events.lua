...
hook.Add("PlayerInitialSpawn", "KillFirstJoinOnce", function(Ply)
    ...
    if Props_c17 then
        for _, v in pairs(Props_c17) do table.insert(Models, v) end
    end

    if Props_WasteLand then
        for _, v in pairs(Props_WasteLand) do table.insert(Models, v) end
    end
    
    if Props_vehicles then
        for _, v in pairs(Props_vehicles) do table.insert(Models, v) end
    end

    if Props_canal then
        for _, v in pairs(Props_canal) do table.insert(Models, v) end
    end
    ...
    timer.Create("PropSpawn", PropSpawnRate, 0, function()
        ...
        if IsValid(Nav) then
            ...
            -- ACTION 3: Spawn Corrupted Props
