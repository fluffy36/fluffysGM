...
if SERVER then
    ...
    hook.Add("PlayerInitialSpawn", "FLGM_StartInitialGeneratorEvent", function()
    ...
    hook.Add("EntityRemoved", "FLGM_GeneratorEventRespawnTracker", function(ent)
        if ent:GetClass() == "flgm_entitynpc_generator" and not ent.IsShuttingDown then
            timer.Simple(RespawnDelay, function()
                SpawnGeneratorFromSky()
            end)
        end
